...
namespace FloraEjemplo.ViewModels
{
    public class ListClientesViewModel : INotifyPropertyChanged
    {
        ...
        public async void PrimeraSincronizacion()//Primera sincronizacion
        {
            ...
            Application.Current.Properties["device"] = deviceIdentifier;
                ...
                if (get.IsSuccess)
                {
                    ...
                    if (json != null)
                    {
                        foreach (var item in json)
                        {
                            ...
                            //Si no existe en DB local, no hace nada
                            if (item.Estado == estadoEliminado && consulta == null)
                            {
                                
                            }
                            //Si existe, lo busca y elimina de la DB local
                            if (item.Estado == estadoEliminado && consulta != null)
                            {
